...
\subsection*{Reader's guide to neurotransmitter lateralization}

Because the central biological claim concerns a molecular property of synaptic input rather than a generic left--right size difference, we define the key terms used below. \textit{Neurotransmitter input profile} denotes the synapse-weighted composition of predicted transmitter classes among all upstream inputs to a neuron. Thus, ``serotonin right-enrichment'' means that right-hemisphere KCs receive a larger fraction of serotonin-predicted upstream input than their matched left-hemisphere counterparts. It does \textit{not} mean that KCs themselves become serotonergic; in fact, KC output synapses are uniformly cholinergic in the FlyWire NT predictions. Conversely, ``glutamate left-bias'' means that left-hemisphere KCs receive more glutamate-predicted upstream input. We use Cohen's $d$ with an R$-$L sign convention, so positive values indicate right enrichment and negative values indicate left enrichment.

\begin{table}[!ht]
\centering\small
\caption{\textbf{Plain-language definitions for the lateralization analysis.}}
\label{tab:reader_guide}
\begin{tabularx}{\linewidth}{@{}p{3.2cm}X@{}}
\toprule
Term & Operational meaning in this study \\
\midrule
Serotonin / 5-HT / Ser & Modulatory transmitter class. The strongest cross-type signal is right-hemisphere enrichment of serotonin-predicted input to KCs. \\
Glutamate / Glu & Transmitter class showing the second most consistent direction: left-hemisphere enrichment in most KC types. \\
DA & Dopamine. In the MB it often carries reward or punishment teaching signals; here it shows concordant right enrichment in $\alpha'\beta'$ types. \\
ACh & Acetylcholine. KC outputs are essentially cholinergic, so the reported asymmetry is an input-side property. \\
KC & Kenyon cell, the principal MB neuron class that encodes odour information sparsely. \\
$\alpha'\beta'$ lobe & Memory-consolidation-related KC lobe system; it shows the strongest serotonin right-enrichment and glutamate left-bias. \\
Laterality index & \((R-L)/(R+L)\); positive values are right-biased and negative values are left-biased. \\
Matched random control & A same-side, same-size random KC set used to test whether a selected lateralized seed ensemble behaves differently from arbitrary KC activation. \\
\bottomrule
\end{tabularx}
\end{table}
...
We then asked whether the transmitter-defined lateralization itself is functionally amplified by the connectome. Two seed ensembles were derived directly from the structural result: right-hemisphere serotonin-enriched KCs and left-hemisphere glutamate-enriched KCs. Each ensemble was propagated through the signed FlyWire graph and compared with same-side, same-size random KC controls. Right-serotonin KC activation significantly changed DAN mass ($z=4.24$), DPM mass ($z=4.02$), APL mass ($z=-4.04$), response laterality ($z=-2.71$) and the maximum target score ($z=-3.83$), with FDR $q=0.038$ for all five readouts. Left-glutamate KC activation produced a broader memory-axis signature: memory-axis mass ($z=3.08$), DAN mass ($z=7.19$), MBIN mass ($z=2.92$), DPM mass ($z=7.18$), MBON mass ($z=-5.35$), APL mass ($z=-4.01$), response laterality ($z=6.99$) and maximum target score ($z=-2.65$) were all significant at FDR $q=0.038$. These results strengthen the NT lateralization claim by showing that transmitter-selected, side-specific KC populations are not interchangeable with arbitrary same-side KCs: the lateralized transmitter signal is projected into memory-axis and modulatory readouts by the connectome. They remain circuit-propagation evidence, not a proof that serotonin or glutamate asymmetry alone causes overt behaviour.

We next tested this prediction in a stricter OCT/MCH assay with side-balanced odour placement. Each condition was run with CS$+$ on both the left and right sides (50 seeds per side, 100 trials per condition; 800 total trials), and trajectories were scored before deletion for approach margin, expected-direction lateral displacement, early lateral velocity, curvature and uncorrected physical left--right drift. The public calibrated motor bridge robustly expressed valence memory: appetitive OCT, appetitive MCH counterbalance, aversive OCT shock and weak-OCT/strong-MCH conflict all showed significant expected choices (expected-choice rates 0.85--0.88; FDR $q=4.83\times10^{-13}$ to $3.82\times10^{-15}$; aversive approach margin $=-0.244$ with expected choice defined as CS$-$). However, unilateral MB perturbations did not survive FDR correction against matched wild type on approach margin, early lateral velocity, expected laterality index or physical laterality (all corrected $q=1.0$). Curvature provided the strongest trend for left-MB silencing and left--right MB swapping (corrected $q=0.171$), but remained non-significant. This negative result constrains the interpretation: the present low-dimensional \texttt{MemoryCondition} bridge validates valence-sensitive odour memory, but is not yet sufficient to demonstrate a behavioural consequence of MB lateralization. The missing mechanistic layer is likely the compartment-specific MBON/DAN/APL/DPM-to-DN interface, rather than further tuning of a scalar lateral-bias term. Representative mirror-side OCT/MCH videos were therefore generated only as explanatory supplementary material. The revised v2 videos are driven directly by saved trajectory CSV files rather than by raw FlyGym video overlays: each frame redraws the arena, OCT/MCH cups, filter papers, odour plumes, sucrose or shock cues, trajectory tail, fly heading and the independent $n=100$ statistics inset.

To make the missing descending interface explicit, we implemented a public MB-to-DN-to-motor readout. MBON, DAN, APL and DPM neurons were selected from the FlyWire mushroom-body annotation table and propagated through the signed FlyWire v783 graph on four GPUs (18 seed conditions, 3 hops, active cap 5,000; runtime 31.8\,s). Responses were then restricted to neurons annotated as descending and grouped by DN family. MBON seeds produced the strongest descending readout: left MBON seeds recruited 202 DNs with total absolute DN mass 0.0648, whereas right MBON seeds recruited 149 DNs with a stronger right-biased laterality index (+0.307). The combined right memory-axis seed set also showed right-biased descending output (+0.420), whereas the left memory-axis seed set was nearly balanced (-0.016). OCT/MCH KC-context seeds produced much weaker descending mass, consistent with an intermediate MB stage before motor output. We then mapped DN-family fractions onto low-dimensional motor primitives using a transparent heuristic interface, not a recovered proprietary DN-to-body controller. This analysis supports a sharper hypothesis: MB output pathways, especially MBON and memory-axis seeds, can access lateralized descending interfaces compatible with turning and memory-expression primitives; causal behavioural claims still require targeted DN or MBON/DAN/APL/DPM perturbation.

After discussion with experimental collaborators, we reframed the next validation step as a set of separable predictions rather than a single claim that all lateralization should collapse into one behavioural score (Fig.~\ref{fig:meeting_feedback}). First, we compared the absolute effect sizes of the right-serotonin and left-glutamate KC seed ensembles across readouts. The left-glutamate ensemble produced the larger absolute perturbation for memory-axis, MBON, MBIN, DAN, DPM and response-laterality metrics, whereas the right-serotonin ensemble produced the larger peak-target effect and a comparable APL effect. Thus, the stricter interpretation is not a completed double dissociation. Instead, left-glutamate is currently the stronger broad memory-output perturbation, while right-serotonin remains a prioritized DPM/5-HT and memory-consolidation validation axis. Second, because the same individual fly cannot be imaged and then behaviourally tested after destructive brain imaging, we translated the model predictions into group-level T-maze observables. In the model, wild-type OCT/MCH memory gives an expected choice index of 0.72; removing the serotonin-right axis reduces the predicted index to 0.60, removing the glutamate-left axis to 0.57, and blunting both to 0.36. These values should be treated as experimentally testable effect directions and approximate magnitudes, not as fitted behavioural parameters. Third, DPM optogenetic stimulation was simulated directly by propagating left and right DPM seed neurons through the signed graph on GPUs 0 and 1. Left DPM activation produced a left-biased propagation readout (LI $=-0.833$), whereas right DPM activation produced a right-biased readout (LI $=+0.807$), mainly recruiting Kenyon cells plus MBIN/MBON/DAN targets. We therefore propose a practical imaging control: if 5-HT release lateralization is a true brain-side signal, rotating the fly horizontally by 180$^\circ$ and registering to brain coordinates should preserve the anatomical sign; an optical angle artifact should flip in image coordinates. Finally, GRASP is treated as the structural hard line: right DPM/5-HT input to $\alpha'\beta'$ KCs and left glutamatergic input to the same lobe system are the highest-priority structural validation pairs. Functional imaging and T-maze statistics can strongly corroborate the model, but do not replace this direct structural test.

We then expanded the DPM optogenetic analysis into a wet-lab-facing protocol screen (Fig.~\ref{fig:dpm_opto_validation}). The screen separates two proofs that should not be forced into the same animal. The first is a functional-imaging proof: DPM-driver-driven red-shifted opsins are paired with KC- or mushroom-body-compartment 5-HT sensors or calcium indicators, and the primary readout is a brain-registered release laterality index after 180$^\circ$ rotation control. The second is a group-behaviour proof: independent cohorts are tested in OCT/MCH T-maze or trajectory assays while DPM is stimulated during training, consolidation or retrieval windows. A protocol library spanning ChR2-blue, ReaChR-red and CsChrimson-red, wavelengths from 470 to 660 nm, 1--40 Hz trains, 5--50 ms pulses and 0.5--30 s durations prioritized red-light protocols to reduce visual confounds. The highest-priority imaging protocols were CsChrimson at 617 nm or ReaChR at 627 nm, 40 Hz, 20 ms pulses for 5 s at 0.1 mW mm$^{-2}$; both yielded a predicted peak brain-registered 5-HT release LI of approximately 0.80. Behaviourally, the model predicted the largest effect in a weak-CS$+$/strong-CS$-$ OCT/MCH conflict assay (choice-index shift $\approx 0.10$), smaller positive shifts in appetitive memory, and a negative shift in shock-associated expression. These are deliberately framed as falsifiable protocol recommendations rather than evidence that optogenetic behaviour has already been measured.
...
\textbf{d},~Synapse-weighted neurotransmitter mass in KC upstream input by hemisphere. Supplementary videos in \texttt{paper/video/food\_memory\_cs\_plus\_left.mp4} and \texttt{paper/video/food\_memory\_cs\_plus\_right.mp4} show mirrored food-odour search simulations; \texttt{paper/video/oct\_mch\_assay\_v2\_key\_conditions.mp4} and \texttt{paper/video/oct\_mch\_assay\_v2\_mb\_perturbations.mp4} show trajectory-driven OCT/MCH assay animations with explicit odour cups, sucrose/shock cues, trajectory tails and formal-statistics insets; \texttt{paper/video/mb\_dn\_motor\_readout\_summary.mp4} illustrates the public MB-to-DN-to-motor interface.}
...
The transmitter-defined propagation suite adds a second bridge between structure and function. Selecting KCs by the observed lateralized transmitter input---right-serotonin-enriched KCs and left-glutamate-enriched KCs---produced significant memory-axis and modulatory readout differences relative to matched random KC controls. This is stronger than a descriptive asymmetry table because it asks whether the asymmetric transmitter composition identifies cell ensembles with distinct downstream influence. At the same time, it is deliberately weaker than a behavioural claim: the result supports functional plausibility and target prioritization, while the causal step still requires either a validated closed-loop DN-to-body controller or wet-lab perturbation.

Taken together, the most stable publishable story has four parts. First, KC inputs show robust NT lateralization, dominated by serotonin right-enrichment and glutamate left-bias, with the strongest effects in the memory-consolidation $\alpha'\beta'$ lobe. Second, this asymmetry is not merely descriptive, because the same side-specific transmitter-defined KC ensembles propagate differently through the connectome and recruit memory-axis, DAN, APL, DPM and MBON/MBIN readouts rather than behaving like matched random KC controls. Third, the current MB perturbation assays provide an important boundary condition: they do not yet produce a significant behavioural effect, which keeps the manuscript rigorous and prevents an over-claim of complete causal closure. Fourth, the OCT/MCH task and its rendered videos provide a reproducible and interpretable behavioural scaffold that is useful for methods, figures and supplementary material even before the final wet-lab validation.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.58\linewidth]{figures/Fig_mb_dn_motor_primitive_heatmap.png}
\includegraphics[width=0.39\linewidth]{figures/Fig_mb_dn_laterality_index.png}
\caption{\textbf{Public MB-to-DN-to-motor interface.} Left, transparent motor-primitive scores derived from DN-family response fractions for MBON, DAN, APL/DPM, memory-axis and OCT/MCH KC-context seeds. Right, MB-to-DN hemispheric laterality index. MBON and combined memory-axis seeds produce the strongest descending readouts; right MBON and right memory-axis seeds show a right-biased descending interface. This figure exposes the surrogate DN-to-motor layer explicitly and should not be interpreted as recovered proprietary Eon motor weights.}
\label{fig:mb_dn_motor}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.48\linewidth]{figures/Fig_meeting_double_dissociation_heatmap.png}
\includegraphics[width=0.48\linewidth]{figures/Fig_dpm_optogenetic_protocol_predictions.png}
\includegraphics[width=0.48\linewidth]{figures/Fig_group_behavior_observable_predictions.png}
\includegraphics[width=0.48\linewidth]{figures/Fig_validation_logic_after_meeting.png}
\caption{\textbf{Meeting-driven validation plan for transmitter lateralization.}
\textbf{a},~Absolute effect-size comparison between right-serotonin and left-glutamate KC seed propagation. Left-glutamate is the stronger broad memory-output perturbation, while right-serotonin remains a DPM/5-HT validation axis.
\textbf{b},~DPM optogenetic protocol predictions across frequency, duration and waveform; true brain-side lateralization should preserve anatomical sign after 180$^\circ$ rotation and brain-side registration, whereas imaging artifacts should flip in image coordinates.
\textbf{c},~Group-level T-maze behavioural predictions expressed as choice index, allowing validation in hundreds of flies without requiring the same individual to undergo destructive brain imaging.
\textbf{d},~Updated validation logic: GRASP provides the structural hard line, DPM optogenetic imaging tests functional stability, and OCT/MCH T-maze assays provide group-level behavioural corroboration.}
\label{fig:meeting_feedback}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.48\linewidth]{figures/Fig_dpm_opsin_wavelength_protocol_space.png}
\includegraphics[width=0.48\linewidth]{figures/Fig_dpm_downstream_roi_heatmap.png}\\[0.5em]
\includegraphics[width=0.48\linewidth]{figures/Fig_dpm_5ht_release_timecourses.png}
\includegraphics[width=0.48\linewidth]{figures/Fig_dpm_behavior_modulation_predictions.png}\\[0.5em]
\includegraphics[width=0.68\linewidth]{figures/Fig_dpm_wetlab_validation_design.png}
\caption{\textbf{DPM optogenetic validation separates release-pattern and behaviour proofs.}
\textbf{a},~Opsin/wavelength protocol space prioritizes red-shifted activation to reduce visual confounds while retaining blue-light positive controls.
\textbf{b},~DPM signed propagation recruits KC, $\alpha'\beta'$ memory-consolidation, MBON, APL and DAN readouts.
\textbf{c},~Predicted 5-HT release time courses and brain-registered release laterality index under the top red-light protocol.
\textbf{d},~Group-level OCT/MCH behavioural predictions; the weak-CS$+$/strong-CS$-$ conflict assay is the most sensitive predicted readout.
\textbf{e},~Wet-lab design: destructive NT imaging and behaviour are separated into independent validation routes, with GRASP retained as the structural hard line.}
\label{fig:dpm_opto_validation}
\end{figure}
...
Descending-neuron family analysis used the same propagated response table. Neurons annotated with super-class ``descending'' were retained and grouped by the leading family prefix in their cell-type label, including DNg, DNge, DNp, DNpe, DNa, DNae, DNbe, DNb, DNde, DNd, DNc, MDN and oviDN classes. For each sensory condition, we computed the number of recruited descending neurons, total absolute descending mass, signed descending mass, family-level absolute mass fractions and a hemispheric laterality index defined as \((M_R-M_L)/(M_R+M_L)\), where \(M_R\) and \(M_L\) are right- and left-side absolute descending response masses. This analysis generated Supplementary Table outputs and a mechanism video under \url{/unify/ydchen/unidit/bio_fly/outputs/dn_behavior_readout}.

For the direct MB-to-DN interface, MB annotations were loaded from \url{/unify/ydchen/unidit/bio_fly/data/processed/flywire_mushroom_body_annotations.parquet}. Rows were classified as MBON, DAN, APL or DPM using the FlyWire cell-class and cell-type labels; APL and DPM were kept separate from generic dopamine labels to preserve their feedback interpretation. Seed conditions were generated for left, right and bilateral MBON, DAN, APL/DPM feedback and combined memory-axis sets. Additional OCT/MCH context conditions used the top KC readout neurons from the glomerulus-level sensory encoder. Each condition was propagated through \texttt{Connectivity\_783.parquet} with the same sparse CUDA signed-propagation backend. Descending responses were grouped by DN family and converted to motor primitives by a fixed, transparent family-to-primitive table: MDN increases avoidance, DNge increases grooming, DNg/DNp/DNpe increase turning and locomotor readout, and MBON or memory-axis seed sets add a memory-expression component. This table is intentionally reported as a surrogate interface and not as an inferred proprietary DN-to-body controller. The implementation and outputs are in \url{/unify/ydchen/unidit/bio_fly/src/bio_fly/mb_dn_motor_readout.py} and \url{/unify/ydchen/unidit/bio_fly/outputs/mb_dn_motor_readout}.

For transmitter-lateralized KC seed propagation, we used the structural lateralization table from \url{/unify/ydchen/unidit/bio_fly/outputs/kc_nt_lateralization/kc_nt_lateralization_effects.csv} and selected side-specific KC ensembles enriched for the two strongest directional transmitter signals: right-serotonin-enriched KCs and left-glutamate-enriched KCs. The selected seeds and subtype contributions are stored in \url{/unify/ydchen/unidit/bio_fly/outputs/model_linkage_gpu/kc_nt_seed_summary.csv}. Each seed ensemble was propagated through the signed FlyWire graph using the same sparse CUDA backend as the MB-to-DN analysis. Null distributions were generated from 128 side-matched random KC seed sets with the same seed count and propagation parameters. Readouts included absolute response mass in the memory axis, MBONs, DANs, MBINs, APL, DPM, response laterality and maximum target score. Two-sided empirical $p$-values were computed from the random-control distribution and corrected across readouts by Benjamini--Hochberg FDR; the resulting table is \url{/unify/ydchen/unidit/bio_fly/outputs/four_card_suite/suite_empirical_significance.csv}.

For the meeting-driven validation analysis, we converted the transmitter propagation outputs into experimentally oriented readouts. Absolute effect sizes, rather than signed $z$ values alone, were used to compare right-serotonin and left-glutamate perturbation strength because the sign of $z$ encodes the direction of a readout change rather than dominance. DPM optogenetic simulations selected the left and right DPM neurons from \url{/unify/ydchen/unidit/bio_fly/outputs/mb_dn_motor_readout/mb_seed_table.csv} and propagated them separately through the FlyWire graph with \texttt{CUDA\_VISIBLE\_DEVICES=0,1}, using \texttt{cuda:0} for left DPM and \texttt{cuda:1} for right DPM. Optogenetic protocol predictions then swept stimulation frequency, duration and duty cycle and converted response asymmetry into a predicted brain-registered 5-HT release laterality index. The 180$^\circ$ rotation control was modelled as preserving anatomical sign for true brain-side lateralization but reversing image-coordinate sign for optical artifacts. Group T-maze predictions used the side-balanced OCT/MCH wild-type expected-choice rate as the baseline and reduced choice index according to the relative absolute propagation effects of serotonin-right and glutamate-left axes. These analyses are implemented in \url{/unify/ydchen/unidit/bio_fly/src/bio_fly/meeting_feedback_experiments.py}; outputs are stored in \url{/unify/ydchen/unidit/bio_fly/outputs/meeting_feedback_20260429}.

For the DPM optogenetic validation suite, we built an explicit protocol library covering ChR2-blue, ReaChR-red and CsChrimson-red action spectra, wavelengths of 470, 530, 590, 617, 627 and 660 nm, train frequencies of 1, 5, 10, 20 and 40 Hz, pulse widths of 5, 10, 20 and 50 ms, durations of 0.5, 1, 2, 5 and 30 s, and irradiance values of 0.05, 0.1, 0.3 and 1.0 mW mm$^{-2}$. Relative opsin activation was modelled with a Gaussian action spectrum centred at the nominal peak wavelength; blue-light protocols were retained as positive controls but penalized for visual-confound risk. DPM propagation responses were grouped into KC, $\alpha'\beta'$ KC, MBON, DAN, APL, DPM and descending-neuron readout classes. A simple release kernel converted protocol energy and propagation laterality into left/right 5-HT release time courses, release area under the curve and brain-registered release LI. Separate lateralized, symmetric and camera-artifact virtual flies were simulated so that the 180$^\circ$ rotation control could distinguish anatomical laterality from image-coordinate artifacts. Behaviour predictions used the OCT/MCH mirror-side formal suite as the baseline and converted release AUC plus release LI into protocol-specific choice-index deltas. The implementation is in \url{/unify/ydchen/unidit/bio_fly/src/bio_fly/dpm_optogenetic_validation.py}; outputs are stored in \url{/unify/ydchen/unidit/bio_fly/outputs/dpm_optogenetic_validation_20260429}, with a mechanism video at \url{/unify/ydchen/unidit/bio_fly/paper/video/dpm_optogenetic_release_prediction.mp4}.
...
All source data from FlyWire (\url{https://flywire.ai}). KC type labels from corrected \url{FlyWire_KC_id_type_color.csv}; hemisphere annotations from \url{classification.csv}; synaptic connections from \url{proofread_connections_783.feather}. All analysis scripts are available at [URL upon publication].
