...
For the direct MB-to-DN interface, MB annotations were loaded from \texttt{/unify/ydchen/unidit/bio\_fly/data/processed/flywire\_mushroom\_body\_annotations.parquet}. Rows were classified as MBON, DAN, APL or DPM using the FlyWire cell-class and cell-type labels; APL and DPM were kept separate from generic dopamine labels to preserve their feedback interpretation. Seed conditions were generated for left, right and bilateral MBON, DAN, APL/DPM feedback and combined memory-axis sets. Additional OCT/MCH context conditions used the top KC readout neurons from the glomerulus-level sensory encoder. Each condition was propagated through \texttt{Connectivity\_783.parquet} with the same sparse CUDA signed-propagation backend. Descending responses were grouped by DN family and converted to motor primitives by a fixed, transparent family-to-primitive table: MDN increases avoidance, DNge increases grooming, DNg/DNp/DNpe increase turning and locomotor readout, and MBON or memory-axis seed sets add a memory-expression component. This table is intentionally reported as a surrogate interface and not as an inferred proprietary DN-to-body controller. The implementation and outputs are in \texttt{/unify/ydchen/unidit/bio\_fly/src/bio\_fly/mb\_dn\_motor\_readout.py} and \texttt{/unify/ydchen/unidit/bio\_fly/outputs/mb\_dn\_motor\_readout}.
